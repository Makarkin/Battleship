...
public class Controller {
    ...
    private void makeShot(MouseEvent mouseEvent) throws ParserConfigurationException, IOException, ClassNotFoundException {
        if (yourTurn) {
            ...
            Document coordinatesOfFire = AuxilaryMethodsXML.writeXMLFire(i, j);
    ...
    private void getShot() throws IOException, ParserConfigurationException, ClassNotFoundException {
        ...
        if (!yourTurn) {
            ...
            int iIndex;
            int jIndex;
            Object enemyShotCoordinates = client.getOis().readObject();
    ...
    @FXML
    private void start(ActionEvent actionEvent) throws ParserConfigurationException {
        System.out.println("Start");
    }

    private void transferFireCoordinates(Document document) throws IOException {
        //отправка клиенту-противнику координаты залпа
        Object obj = (Object) document;
        client.getOos().writeObject(obj);
    }

    private boolean acceptResultOfFireController() throws IOException, ClassNotFoundException {
        //принятие результата вашего залпа
        Object object = client.getOis().readObject();
